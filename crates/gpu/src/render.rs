...
use tazama_core::{EffectKind, ProjectSettings, Timeline, TrackKind, TransitionKind, keyframe};
...
use crate::lut;
use crate::pipeline::{
    ColorGradePush, CompositePush, CropPush, LutPush, PipTransformPush, PipelineCache,
    TransitionPush,
};
use crate::text;
    ...
    pub fn render_frame(
        ...
        for clip_info in &active_clips {
            ...
            let source_frame = if let Some(ref speed_track) = clip_info.speed_keyframe_track {
                ...
                clip_info.source_offset + integrated as u64
            } else {
                // Constant speed
            ...
            for effect in &clip_info.effects {
                ...
                // Resolve keyframed parameters: for each keyframe track,
                // evaluate at the current frame and override the static value.
                let resolve_param = |name: &str, static_val: f32| -> f32 {
                ...
                match &effect.kind {
                    EffectKind::ColorGrade {
                        ...
                        let brightness = resolve_param("brightness", *brightness);
                        let contrast = resolve_param("contrast", *contrast);
                        let saturation = resolve_param("saturation", *saturation);
                        let temperature = resolve_param("temperature", *temperature);
                    ...
                    EffectKind::Crop {
                        ...
                        let left = resolve_param("left", *left);
                        let top = resolve_param("top", *top);
                        let right = resolve_param("right", *right);
                        let bottom = resolve_param("bottom", *bottom);
                    ...
                    EffectKind::Lut { lut_path } => {
                        ...
                        let lut_content = std::fs::read_to_string(lut_path).map_err(|e| {
                            GpuError::Other(format!("failed to read LUT file: {e}"))
                        })?;
                    ...
                    EffectKind::Transform {
                        ...
                        self.clear_buffer(&base, frame_size)?;
                    ...
                    EffectKind::Text {
                        ...
                        let max_w = width;
                        let max_h = height;
                        let (text_rgba, text_w, text_h) = text::rasterize_text(
                            content,
                            font_family,
                            *fsize,
                            *color,
...
struct ActiveClip {
    ...
    speed_keyframe_track: Option<tazama_core::KeyframeTrack>,
